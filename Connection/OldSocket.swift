...
enum OldSocketError : ErrorType, CustomStringConvertible {
...
public class OldSocket {
    ...
    init() throws {
        ...
        if connection < 1 {
            throw OldSocketError.Connection(__FUNCTION__, errno)
        }
    ...
    func bind(port:Port) throws {
        ...
        guard socket_bind(connection, pointer, length) < 0 else {
            throw OldSocketError.Bind(__FUNCTION__, errno)
        }
    }
    
    func listen() throws {
        if socket_listen(connection, 0) < 0 {
            throw OldSocketError.Listen(__FUNCTION__, errno)
        }
    }
    
    func accept() throws -> OldSocket {
        let incoming = socket_accept(connection, nil, nil)
        if incoming < 0 {
            throw OldSocketError.Accept(__FUNCTION__, errno)
        }
        
        return OldSocket(connection: connection)
    }
